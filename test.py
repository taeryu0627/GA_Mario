...
class MyApp(QWidget):
    def __init__(self):
        ...
        self.screen_label.setGeometry(1080-screen_width, 0, 1080, screen_height)
...
# screen 파트
        ...
        # 타일 화면 오프셋
        screen_offset = screen_offset // 16

        # 현재 화면 추출
        screen_tiles = np.concatenate((full_screen_tiles, full_screen_tiles), axis=1)[:,screen_offset:screen_offset + 16]
...
# enemy 파트
        ...
        for h in range(map_height):
            for w in range(map_width):
                ...
                painter.drawRect((screen_offset + 16) % 32 * 15, h * 15, 15, 15)
                painter.setBrush(QBrush(Qt.blue))
                painter.drawRect(screen_offset % 32 * 15, h * 15, 15, 15)
                painter.setBrush(QBrush(Qt.red))
                painter.drawRect((screen_offset + player_tile_position_x) % 32 * 15, player_tile_position_y * 15, 15, 15)
