...
class Chromosome:
    ...
    def fitness(self):
        ...
        # fit = int(fit)
        # return fit
        return int(max(self.distance ** 1.8 - self.frames ** 1.5 + min(max(self.distance - 50, 0), 1) * 1000 + self.win * 1000000, 1))
...
# frame
        ...
        self.input_data = screen_tiles[frame_y:frame_y + 10, frame_x:frame_x + 8]

        if 2 <= player_tile_position_y <= 11:
            self.input_data[player_tile_position_y - 2][0] = 2
    ...
    def frame(self):
        ram = self.env.get_ram()
        self.input_data = self.input_data.flatten()
        ...
        if player_float_state == 0x03 or player_state in (
                0x06, 0x0B) or player_vertical_screen_position >= 2 or current_chromosome.stop_frames > 180:
            ...
            predict = current_chromosome.predict(self.input_data)
