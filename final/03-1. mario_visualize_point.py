...
# 위에 있는 원
            ...
            for i in range(current_chromosome.l1.shape[0]):
                if current_chromosome.l1[i] == 0:
                    ...
                    painter.setBrush(QBrush(Qt.blue))
                painter.drawEllipse(self.neural_network_l1_margin_x + self.screen_width + i * 40, 240, 10 * 2, 10 * 2)
# 아래 있는 원
            for i in range(6):
                if predict[i] == 1:
                    ...
                    painter.setBrush(QBrush(Qt.gray))
                painter.drawEllipse(self.neural_network_predict_margin_x + self.screen_width + i * 40 - 10, 440, 14 * 2, 14 * 2)
                text = ('U', 'D', 'L', 'R', 'A', 'B')[i]
                painter.drawText(self.neural_network_predict_margin_x + self.screen_width + i * 40 - 20, 468, text)

        painter.drawText(self.neural_network_predict_margin_x + self.screen_width + 220, 460,
                         str(self.ga.generation) + ' 세대')
        painter.drawText(self.neural_network_predict_margin_x + self.screen_width + 220, 470,
                         str(current_chromosome.fitness()))
